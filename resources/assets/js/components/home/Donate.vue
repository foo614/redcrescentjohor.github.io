    ...
    <div>
        <v-container fluid grid-list-md v-if="!submitCompleted">
            <v-card class="mb-3">
                <v-layout row wrap>
                    <v-flex xs12 sm6>
                    <v-card-title primary-title><div class="headline"> {{selectedFundraiser.title}}</div>
                    <v-spacer></v-spacer>
                        <v-dialog v-model="dialog" persistent max-width="800px">
                            <v-btn color="#ca0000" round large slot="activator" dark>Donate Now</v-btn>
...
export default {
  ...
  methods: {
    totalDonationValue() {
      let app = this;
      app.donationProgress =
        (app.totalDonation / app.selectedFundraiser.target_amount) * 100;
    },
    saveItem() {
