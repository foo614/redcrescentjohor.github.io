<template>
    <div>
        <v-navigation-drawer :clipped="$vuetify.breakpoint.mdAndUp" app right v-model="drawer" temporary width=250>
            <v-list dense>
                <!-- <v-list-tile v-for="item in items" :key="item.text" avatar>
                    <v-list-tile-action>
                        <v-icon v-if="item.icon">{{item.icon}}</v-icon>
                    </v-list-tile-action>
                    <v-list-tile-title>{{item.text}}</v-list-tile-title>
                </v-list-tile> -->
                <template v-for="item in items" avatar>
                    <v-list-group
                        v-if="item.children"
                        v-model="item.model"
                        :key="item.text"
        ...
        <v-toolbar :clipped-right="$vuetify.breakpoint.mdAndUp" app fixed color="white" height="78">
            <a href="/"><img src="/img/64x64.png" height="38px" width="38px"></a>
            <a href="/" style="font-weight: 500; font-size: 18px; text-decoration:none; color:black"><v-toolbar-title>Red Crescent Johor</v-toolbar-title></a>
            <v-spacer></v-spacer>
    ...
    data: () => ({
        ...
        items: [{
                ...
                },
                {
                    ...
                    link: "/",
                    children: [
                        { text: "News", link: "/users/create" },
                        { text: "Stories", link: "/users" },
                        { text: "Memories", link: "/users" }
                    ]
                },
                {
