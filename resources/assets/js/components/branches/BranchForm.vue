    ...
    <v-form ref="form" v-model="valid" lazy-validation @submit.prevent="saveItem">
        <v-card>
            ...
            <v-card-title primary-title>
                <div class="headline">{{$route.name == "editBranch" ? 'Edit' : 'Add' }} Branch</div>
            </v-card-title>
...
export default {
    ...
    mounted() {
        ...
        if(this.$route.name === "editBranch"){
            ...
            .then(function (res) {
                app.item = res.data
                app.item.branch_id = res.data.id
            })
            .catch(function () {
    ...
    methods:{
        saveItem(){
            if (this.$refs.form.validate()){
                ...
                setTimeout(()=> {
                    fetch("/api/branch", {
                        method: this.$route.name == 'createBranch' ? "post" : "put",
                    ...
                    .then(res => {
                        ...
                        this.$router.push({ path: '/branches' }, ()=> {
                            this.$toasted.success(this.item.name + (currentPage === 'createBranch' ? ' added' : ' updated') , {icon:"check"})
                        })
                    })
